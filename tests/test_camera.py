...
def test_camera_constructor():
    ...
    assert c.transform == create_identity_matrix()

def test_pixel_size_horizontal_canvas():
    c = Camera(200, 125, math.pi/2)
    assert math.isclose(c.pixel_size, 0.01, abs_tol=ABS_TOL)
...
def test_ray_when_camera_transformed():
    c = Camera(201, 101, math.pi/2)
    c.transform = rotation_y(math.pi/4).multiply(translation(0, -2, 5))
    r = c.ray_for_pixel(100, 50)
    assert r.origin == Point(0, 2, -5)
    assert r.direction == Vector(1/math.sqrt(2), 0, -1/math.sqrt(2))
...
def test_render_world():
    ...
    canvas = c.render(w)
    assert canvas.pixel_at(5, 5) == Color(0.38066, 0.47583, 0.2855)
